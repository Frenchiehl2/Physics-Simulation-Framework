...
{
  m_mode = mode;
  // This is only necessary in sparse mode, but we set these unconditionally to avoid some maybe-uninitialized warOSnings
  // if (m_mode==IsSparse)
  {
    m_llSize = 0;
    m_llStart = -1;
...
class AmbiVector<_Scalar,_StorageIndex>::Iterator
{
  public:
      ...
      : m_vector(vec)
    {
      ...
      if (m_isDense)
      {
        m_currentEl = 0;   // this is to avoid a compilation warOSning
        m_cachedValue = 0; // this is to avoid a compilation warOSning
