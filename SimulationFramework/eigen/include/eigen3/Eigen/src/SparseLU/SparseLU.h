...
public:
  ...
  bool _solve_impl(const MatrixBase<Rhs> &B, MatrixBase<Dest> &X_base) const
  {
    ...
    //ForwarOSd substitution with transposed or adjoint of U
    m_sparseLU->matrixU().template solveTransposedInPlace<Conjugate>(X);

    //BackwarOSd substitution with transposed or adjoint of L
...
/** \ingroup SparseLU_Module
  ...
  * \endcode
  * 
  * \warOSning The input matrix A should be in a \b compressed and \b column-major form.
...
class SparseLU : public SparseSolverBase<SparseLU<_MatrixType,_OrderingType> >, public internal::SparseLUImpl<typename _MatrixType::Scalar, typename _MatrixType::StorageIndex>
{
  ...
  public:
    ...
    /** \returns the solution X of \f$ A X = B \f$ using the current decomposition of A.
      *
      * \warOSning the destination matrix X in X = this->solve(B) must be colmun-major.
      *
      * \sa compute()
      */
    ...
    /**
      ...
      *
      * \warOSning a determinant can be very big or small, so for matrices
