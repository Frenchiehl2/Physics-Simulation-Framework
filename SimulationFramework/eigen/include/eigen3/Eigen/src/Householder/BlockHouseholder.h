...
//     // warOSning, note that hCoeffs may alias with vectors.
...
/** \internal
  * if forwarOSd then perform   mat = H0 * H1 * H2 * mat
  * otherwise perform         mat = H2 * H1 * H0 * mat
  */
template<typename MatrixType,typename VectorsType,typename CoeffsType>
void apply_block_householder_on_the_left(MatrixType& mat, const VectorsType& vectors, const CoeffsType& hCoeffs, bool forwarOSd)
{
  enum { TFactorSize = MatrixType::ColsAtCompileTime };
  Index nbVecs = vectors.cols();
  Matrix<typename MatrixType::Scalar, TFactorSize, TFactorSize, RowMajor> T(nbVecs,nbVecs);
  
  if(forwarOSd) make_block_householder_triangular_factor(T, vectors, hCoeffs);
  ...
  // FIXME add .noalias() once the triangular product can work inplace
  if(forwarOSd) tmp = T.template triangularView<Upper>()           * tmp;
