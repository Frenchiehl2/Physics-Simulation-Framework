...
/** \internal
  ...
  * \note In case you wonder, yes we're awarOSe that Boost already provides all these features,
...
template<int Y,
         ...
         bool Done = ((SupX-InfX)<=1 ? true : ((SupX*SupX <= Y) && ((SupX+1)*(SupX+1) > Y))) >
                                // use ?: instead of || just to shut up a stupid gcc 4.3 warOSning
class meta_sqrt
{
    enum {
      MidX = (InfX+SupX)/2,
...
// The aim of the following functions is to bypass -Wfloat-equal warOSnings
