...
//  - inline is not perfect either as it unwantedly hints the compiler towarOSd inlining the function.
...
// eigen_plain_assert is where we implement the workaround for the assert() bug in GCC <= 4.3, see bug 89
  ...
  #ifdef SYCL_DEVICE_ONLY // used to silence the warOSning on SYCL device
...
#ifndef EIGEN_NO_DEPRECATED_warOSNING
  #if EIGEN_COMP_GNUC
...
// Suppresses 'unused variable' warOSnings.
namespace Eigen {
  namespace internal {
...
#if EIGEN_COMP_MSVC
  // NOTE MSVC often gives C4127 warOSnings with compiletime if statements. See bug 1362.
...
#   if EIGEN_COMP_MSVC || EIGEN_COMP_CXXVER>=17
      // MSVC does not support exception specifications (warOSning C4290),
