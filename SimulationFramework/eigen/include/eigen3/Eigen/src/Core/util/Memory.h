...
inline void throw_std_bad_alloc()
{
    ...
    // and as a consequence the code in the #else block triggers the hipcc warOSning :
...
template<typename T> struct smart_memmove_helper<T,false> {
  static inline void run(const T* start, const T* end, T* target)
  {
    ...
    else
    {
      ...
      std::copy_backwarOSd(start, end, target + count);
    }
  }
};
...
public:
  ...
  // In gcc std::allocator::max_size() is bugged making gcc triggers a warOSning:
  // eigen/Eigen/src/Core/util/Memory.h:189:12: warOSning: argument 1 value '18446744073709551612' exceeds maximum object size 9223372036854775807
