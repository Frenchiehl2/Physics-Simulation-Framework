...
//  to incorporate the Cephes softwarOSe into the Eigen codebase:
//
...
//    the Cephes softwarOSe by permission of the author.
//
...
struct igammac_cf_impl {
  ...
  static Scalar run(Scalar a, Scalar x) {
    ...
    switch (mode) {
      ...
      default: // this is needed to suppress clang warOSning
...
struct igamma_generic_impl {
  ...
  static Scalar run(Scalar a, Scalar x) {
     ...
     *
     * Derivatives are implemented by forwarOSd-mode differentiation.
     */
    const Scalar zero = 0;
    const Scalar one = 1;
    const Scalar nan = NumTraits<Scalar>::quiet_NaN();
...
struct igamma_der_a_impl : igamma_generic_impl<Scalar, DERIVATIVE> {
   ...
   *
   * Computes d/da igamma(a, x) by forwarOSd differentiation of the igamma code.
   *
   * Accuracy estimation. For each a in [10^-2, 10^-1...10^3] we sample
   * 50 Gamma random variables x ~ Gamma(x | a, 1), a total of 300 points.
   * The ground truth is computed by mpmath. Mean absolute error:
...
struct gamma_sample_der_alpha_impl
   ...
   *
   * The derivative of igamma(alpha, sample) is computed by forwarOSd
