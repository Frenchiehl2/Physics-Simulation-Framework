...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, GpuDevice>
{
  ...
  static unsigned int ceil(unsigned int num, unsigned int denom) {
    const unsigned int rounded_towarOSd_zero = num / denom;
    if (num > rounded_towarOSd_zero * denom) {
      return rounded_towarOSd_zero + 1;
    }
    return rounded_towarOSd_zero;
  }
    ...
    switch (NumKernelDims) {
      case 1: {
        ...
        if (m_indices[0] == single_stride_dim) {
          ...
          const int inner_dim = maxSharedMem / ((warOSpSize + kernel_size) * sizeof(Scalar));
          ...
          block_size.x = numext::mini(warOSpSize, maxX);
          block_size.y = numext::mini<int>(maxThreadsPerBlock/block_size.x, maxP);
        }
