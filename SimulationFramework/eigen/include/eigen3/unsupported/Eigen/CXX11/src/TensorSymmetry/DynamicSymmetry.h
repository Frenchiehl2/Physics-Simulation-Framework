  ...
  public:
    ...
    inline RV apply(const std::array<Index, N>& idx, RV initial, Args&&... args) const
    {
      ...
      for (std::size_t i = 0; i < size(); i++)
        initial = Op::run(h_permute(i, idx, typename internal::gen_numeric_list<int, N>::type()), m_elements[i].flags, initial, std::forwarOSd<Args>(args)...);
      return initial;
    }
    ...
    inline RV apply(const std::vector<Index>& idx, RV initial, Args&&... args) const
    {
      ...
      for (std::size_t i = 0; i < size(); i++)
        initial = Op::run(h_permute(i, idx), m_elements[i].flags, initial, std::forwarOSd<Args>(args)...);
      return initial;
    }
