...
namespace test_1_3
{
	...
	int test_heat_sim(Prog1_3Simulation & simulation)
	{
		...
		auto i_center = GRID_LENGTH / 2, j_center = GRID_LENGTH / 2;

		// check size of return value
		{
			auto u = simulation.step();

			if(u.size() != GRID_LENGTH * GRID_LENGTH)
			{
				std::fprintf(stderr, "ERROR: Output size and GRID_LENGTH mismatch. Expected: %zi, not %zi", GRID_LENGTH * GRID_LENGTH, u.size());
				return 1;
			}
		}
		...
		// one step
		{
			...
			if(std::fabs(u1[i_center * GRID_LENGTH + j_center] - 0.729573) > 1e-3)
			{
				std::fprintf(stderr, "ERROR: Incorrect heat value at center after first step. Expected: %f, not %f. \n", 0.729573, u1[i_center * GRID_LENGTH + j_center]);
				return 1;
			}

			if(std::fabs(u1.sum() - 1) > 1e-3)
			{
				std::fprintf(stderr, "ERROR: Incorrect total heat value. Expected %f, not %f\n", 1.f, std::fabs(u1.sum()));
				return 1;
			}
		}
		...
		// 20 steps
		{
			...
			for(int i = 0; i < 20; ++i)
			{
				u = simulation.step();
				// check total heat
				if(std::fabs(sumPrev - u.sum()) > 1e-3)
				{
					std::fprintf(stderr, "ERROR: Energy loss after %i steps. Total heat now: %f was %f \n", i, u.sum(), sumPrev);
					return 1;
				}
			}

			// check value at center
			auto u_center = u[i_center * GRID_LENGTH + j_center];
			if(std::fabs(u_center - 0.0431238) > 1e-3)
			{
				std::fprintf(stderr, "ERROR: Unexpected heat value at center point. Expected: %f, not %f \n", 0.0431238, u_center);
				return 1;
			}
			...
			for(int i = 1; i < GRID_LENGTH - 2; ++i)
			{
				...
				{
					sum += u[i * GRID_LENGTH + j];
				}
			}

			if(std::fabs(sum - 1) > 1e-3)
			{
				std::fprintf(stderr, "ERROR: Dirichlet boundary not zero.");
				return 1;
			}
		}
